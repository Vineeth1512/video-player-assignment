...
for (var i = 0; i < playlistData.length; i++) {
    if (i == 0) {
        ...
        <div onclick="applyActiveCard(${playlistData[i].id})" id="card${playlistData[i].id}" class="playlist-card active-card">
        ...
        playListWrapper.innerHTML += `
        <div onclick="applyActiveCard(${playlistData[i].id})" id="card${playlistData[i].id}" class="playlist-card ">
        <img class="thumbnail" src=${playlistData[i].thumbnail} />
        <h3 class="video-card-title">${playlistData[i].title}</h3>
        </div>`
    }
...
function applyActiveCard(id) {
    ...
    var videoPlayer = document.getElementById("player-wrapper");

    videoPlayer.innerHTML = `
<iframe id="video-player" src="https://player.vimeo.com/video/${videoPlaySectionData[id - 1].vimeoId}"
 frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen>
 </iframe>
<div>
    <div id="video-actions">
        <p><span id="views-count">${videoPlaySectionData[id - 1].views / 1000}</span> views</p>
        <div>
    ...
    <p id="video-description">${videoPlaySectionData[id - 1].description}</div>
    `
